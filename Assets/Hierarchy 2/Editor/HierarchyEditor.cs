        ...
        void CustomRowBackground()
        {
            ...
            for (int i = 0; i < settings.instantBackgroundColors.Count; ++i)
            {
                ...
                )
                {
                    ...
                    content = new GUIContent(rowItem.name.Remove(0, settings.instantBackgroundColors[i].startWith.Length));
                }
            }
            ...
            // rect.x += (rect.width - Styles.lineStyle.CalcSize(content).x) / 2;
            rect.x += 45;
            GUI.color = instantBackgroundColor.color==Color.black?Color.white: ThemeData.colorHeaderTitle;
